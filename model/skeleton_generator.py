...
class Generator22(nn.Module):
    def __init__(self, device, parents=None, channels_per_joint=64, kernel_size=3):
        ...
        neighbor_list = [neighbor_list1, neighbor_list2, neighbor_list3, neighbor_list22, neighbor_list22]
        number_layers = len(neighbor_list)

        in_joint_num = [8, 9, 14, 22, 22]
        out_joint_num = [9, 14, 22, 22, 22]

        in_channel_list = [channels_per_joint, channels_per_joint // 2, channels_per_joint // 4,
                           channels_per_joint // 8, channels_per_joint // 8]
        out_channel_list = [channels_per_joint // 2, channels_per_joint // 4, channels_per_joint // 8,
                            channels_per_joint // 8, 6]

        for i in range(number_layers):
            seq = []
            seq.append(
                SkeletonConv(
                    neighbor_list=neighbor_list22,
                    kernel_size=kernel_size,
                    in_channels_per_joint=6,
                    out_channels_per_joint=6,
                    in_joint_num=22,
                    out_joint_num=22,
                    padding=padding,
                    stride=1,
