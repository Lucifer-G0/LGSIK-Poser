...
class Generator(nn.Module):
    def __init__(self, device, parents=None, channels_per_joint=9, kernel_size=3):
        ...
        neighbor_list8 = [[0, 1, 2, 3, 4, 5, 7], [0, 1, 6], [0, 2, 6], [0, 3, 6], [0, 4, 7], [0, 5, 7], [1, 2, 3, 6],
                          [0, 4, 5, 7]]
        neighbor_list6 = [[0, 1, 2, 3, 4, 5, 7], [0, 1, 6], [0, 2, 6], [0, 3, 6], [0, 4, 7], [0, 5, 7]]
        neighbor_list = [neighbor_list8, neighbor_list8, neighbor_list6]
...
class Generator22(nn.Module):
    def __init__(self, device, parents=None, channels_per_joint=64, kernel_size=3):
        """
        8个节点拓展到22个节点
        :param channels_per_joint >=48 前三层每层变为1/2
        """
        ...
        neighbor_list1 = [[0, 7], [2, 7], [3, 7], [0, 1, 7], [1, 6], [0, 4, 6], [4], [0, 5, 6], [5, 6]]
        neighbor_list2 = [[0], [0, 1], [1], [0, 2], [2], [0, 3, 5, 7], [3, 4], [4], [0, 5, 6], [5, 6], [6], [0, 7, 8],
                          [7, 8], [8]]
        neighbor_list3 = [[0], [0, 1], [0, 3], [0, 5], [0, 1, 2], [0, 3, 4], [0, 5, 8, 11], [1, 2], [3, 4], [5, 8, 11],
                          [2], [4], [5, 6, 9], [5, 8, 9], [5, 11, 12], [7], [8, 9], [11, 12], [8, 9, 10], [11, 12, 13],
                          [10], [13]]

        neighbor_list22 = find_neighbor(parents, 2, add_displacement=False)
        neighbor_list = [neighbor_list1, neighbor_list2, neighbor_list3, neighbor_list22, neighbor_list22, neighbor_list22]
        number_layers = len(neighbor_list)

        in_joint_num = [8, 9, 14, 22,22,22]
        out_joint_num = [9, 14, 22, 22,22,22]

        in_channel_list = [channels_per_joint, channels_per_joint // 2, channels_per_joint // 4, channels_per_joint // 8,channels_per_joint // 8,channels_per_joint // 8]
        out_channel_list = [channels_per_joint // 2, channels_per_joint // 4, channels_per_joint // 8,channels_per_joint // 8,channels_per_joint // 8, 6]
        ...
        for i in range(number_layers):
            ...
            seq.append(
                SkeletonConv(
                    neighbor_list=neighbor_list[i],
                    kernel_size=kernel_size,
                    in_channels_per_joint=in_channel_list[i],
                    out_channels_per_joint=out_channel_list[i],
                    in_joint_num=in_joint_num[i],
                    out_joint_num=out_joint_num[i],
                    padding=padding,
                    stride=1,
    ...
    def forward(self, x_in):
        ...
        :param x_in (batch_size,frames,joint_num,channels_per_joint)
        :return: A tuple containing:
         - result: (batch_size,frames, joint_num,channels_per_joint)
        """
        batch_size = x_in.shape[0]
        time_seq = x_in.shape[1]
        x = x_in.reshape(batch_size, time_seq, -1).permute(0, 2, 1)  # batch_size,joint_num*channels_per_joint,frames
        ...
        # change input to shape (batch_size, frames, num_joints, channels_per_joint)
        result = x.permute(0, 2, 1).reshape(batch_size, time_seq, 22, -1)
