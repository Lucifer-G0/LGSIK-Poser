    ...
    for input_type in configs.compatible_inputs:
        ...
        with torch.no_grad():
            for _, (input_feat, gt_local_pose, gt_global_pose, gt_positions, gt_betas, gt_sparse_offsets) in tqdm(
                ...
                input_feat, gt_local_pose, gt_positions, gt_betas, gt_sparse_offsets = input_feat.to(
                    device).float(), gt_local_pose.to(device).float(), \
        ...
        for key in log:
            log[key] /= len(test_dataloader)

        # 保存每个input的log结果
        all_logs.append(log)

        tb.add_row([input_type] +
                   ['%.2f' % (log[metric] * metrics_coeffs[metric]) for metric in pred_metrics] +
                   ['%.2f' % (log[metric] * metrics_coeffs[metric]) for metric in gt_metrics]
                   )
