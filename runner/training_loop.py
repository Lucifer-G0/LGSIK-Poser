...
def train_loop(configs, device, model, loss_func, optimizer, \
    ...
    for epoch in range(configs.train_config.epochs):
        ...
        for batch_idx, (input_feat, gt_local_pose, gt_global_pose, gt_positions, gt_betas,gt_sparse_offsets) in tqdm(
        ...
        epoch_info = {
            ...
            'root_rot_loss': round(float(one_epoch_root_loss), 3),
        ...
        if epoch % configs.train_config.val_interval == 0:
            ...
            with torch.no_grad():
                for _, (input_feat, gt_local_pose, gt_global_pose, gt_positions, gt_betas,gt_sparse_offsets) in tqdm(
                    ...
                    input_feat, gt_local_pose, gt_positions, gt_sparse_offsets = input_feat.to(device).float(), \
                        gt_local_pose.to(device).float(),gt_positions.to(device).float(),gt_sparse_offsets.to(device).float()
                    if len(input_feat.shape) == 4:
                        input_feat = torch.flatten(input_feat, start_dim=0, end_dim=1)
                        gt_local_pose = torch.flatten(gt_local_pose, start_dim=0, end_dim=1)
                        gt_positions = torch.flatten(gt_positions, start_dim=0, end_dim=1)
            ...
            one_epoch_total_loss = round((position_error_+local_pose_error_) * 10000)

            lr_scheduler.step(one_epoch_total_loss)
            ...
            epoch_info = {
                'type': 'test',
                'epoch': epoch + 1,
                'total_loss':one_epoch_total_loss,
