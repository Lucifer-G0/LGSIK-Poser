...
def main():
    ...
    dmpl_fname_male = os.path.join('./body_models', "dmpls/{}/model.npz".format("male"))
    ...
    for input_type in configs.compatible_inputs:
        ...
        with torch.no_grad():
            for _, (
                ...
                filepath = filepath[0]  # dataloader auto tranfer str to tuple, recover
                ...
                for (input_batch, gso_batch) in input_loader:  # (window_batch_size,window_size,D)
                    ...
                    vertices = vertices.reshape(-1, configs.input_motion_length, 6890, 3)
                ...
                pred_shapes = torch.cat(pred_betas, dim=0).unsqueeze(dim=0)  # 1,frames-39,D
                pred_joint_position = torch.cat(pred_positions, dim=0).unsqueeze(dim=0)
                vertices = torch.cat(pred_vertices, dim=0)
                ...
                pred_local_pose_aa = utils_transform.sixd2aa(pred_local_pose.reshape(-1, 6)).reshape(batch_size * time_seq, 22 * 3)
                gt_local_pose_aa = utils_transform.sixd2aa(gt_local_pose.reshape(-1, 6)).reshape(batch_size * time_seq,22 * 3)
                ...
                framerate = motion_data.get('mocap_frame_rate', motion_data.get('mocap_framerate', None))
                if framerate == 120:
                    ...
                    print(motion_data['mocap_framerate'])
                motion_data['framerate'] = 60
                # motion_data['dmpls'] = motion_data['dmpls'][::stride][40:, :]
                motion_data['poses'] = motion_data['poses'][::stride][40:, :]
                motion_data['trans'] = motion_data['trans'][::stride][40:, :]
                motion_data['betas'] = np.tile(motion_data['betas'], (pred_joint_position.shape[0], 1))

                gt_bm = bm_male if motion_data['gender'] == 'male' else bm_female
                body_pose = gt_bm(root_orient=torch.Tensor(motion_data['poses'][:, :3]).to(device),
                                  pose_body=torch.Tensor(motion_data['poses'][:, 3:66]).to(device),
                                  trans=torch.Tensor(motion_data['trans']).to(device),
                                  betas=torch.Tensor(motion_data['betas']).to(device))
                gt_joints = body_pose.Jtr[:, :22]
                gt_vertices = body_pose.v

                motion_data['joints'] = gt_joints.cpu().numpy()
                motion_data['vertices'] = gt_vertices.cpu().numpy()
                np.savez("eval/" + filename + ".npz", **motion_data)

                # for visual align
                head_align_shift = - pred_joint_position[:, 15:16] + gt_joints[:, 15:16]
                pred_joint_position = pred_joint_position + head_align_shift
                vertices = vertices + head_align_shift

                predicted_angle = pred_local_pose_aa[..., 3:66]
                predicted_root_angle = pred_local_pose_aa[..., :3]
                predicted_position = pred_joint_position

                motion_data['poses'][:, :66] = pred_local_pose_aa.cpu().numpy()
                motion_data['trans'] = predicted_position[:, 0, :].cpu().numpy()
                motion_data['betas'] = pred_shapes[0, :].cpu().numpy()
                motion_data['joints'] = predicted_position.cpu().numpy()
                motion_data['vertices'] = vertices.cpu().numpy()
                np.savez("eval/LGSIK_" + filename + ".npz", **motion_data)

                # for eval
                head_align_shift = - pred_joint_position[:, 15:16] + gt_positions[:, 15:16]
                predicted_position = pred_joint_position + head_align_shift
                ...
                tb.add_row([filename] +
                           ['%.2f(%.2f)' % (eval_log[metric] * metrics_coeffs[metric],
                                            eval_log['std_' + metric] * metrics_coeffs[metric]) for
                            metric in pred_metrics] +
                           ['%.2f(%.2f)' % (eval_log[metric] * metrics_coeffs[metric],
                                            eval_log['std_' + metric] * metrics_coeffs[metric]) for
                            metric in gt_metrics]
                           )
