...
class TrainDataset(Dataset):
    def __init__(
        ...
        self.motions = []
        self.window_step = 16
        self.window_size = input_motion_length
        self.motions_num = 0

    def get_motions_num(self):
        return self.motions_num
    ...
    def load_data(self, dataset_path, split):
        ...
        self.motions_num = len(motion_raw_data)
        for data in motion_raw_data:
            if split == "train" and data["rotation_local_full_gt_list"].shape[0] < self.window_size:
                continue
            ...
            for start in range(0, frames, self.window_step):
                end = start + self.window_size
                if end < frames:
                    self.motions.append({
                        'input_feat': input_feat[start:end],
                        'gt_local_pose': gt_local_pose[start:end],
                        'gt_global_pose': gt_global_pose[start:end],
                        'gt_positions': gt_positions[start:end],
                        'gt_betas': gt_betas[start:end]
                    })
